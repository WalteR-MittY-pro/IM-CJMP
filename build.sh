...
if [[ "${buildTarget}" == "android" ]]; then
    ...
    export ANDROID_PACKAGE="$SCRIPT_DIR/package/target/${cangjieTarget}/${buildType}/telegram_cjmp"
    os_name=$(uname -s)
...
elif [[ "${buildTarget}" == "ohos" ]]; then
    cangjieTarget="aarch64-linux-ohos"
    ...
    export IOS_PACKAGE="$SCRIPT_DIR/package/target/${cangjieTarget}/${buildType}/telegram_cjmp"
...
if [[ "${buildTarget}" == "android" ]]; then
    ...
    dep_file="$(analyze_dependencies "$outputDir/arm64-v8a" "android" "$ANDROID_CANGJIE_PATH" "$ANDROID_CJ_FRONTEND" "$ANDROID_CANGJIE_STDX_PATH")"
    ...
    # copy: library dependencies
    dep_file="$(analyze_dependencies "$frameworksDir" "ios" "$IOS_CANGJIE_PATH" "$IOS_CJ_FRONTEND" "$IOS_CANGJIE_STDX_PATH")"
    ...
    # copy: library dependencies
    dep_file="$(analyze_dependencies "$frameworksDir" "ios" "$IOS_SIM_CANGJIE_PATH" "$IOS_SIM_CJ_FRONTEND" "$IOS_SIM_CANGJIE_STDX_PATH")"
    copy_dependencies "$frameworksDir" "$dep_file"
fi